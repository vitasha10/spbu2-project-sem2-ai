...
def build():
    ...
    s = work_slide('Срок до заезда — сильнейший одиночный сигнал',
                   title_width=8500000)
    txt = _textbox(s, LEFT_MARGIN, Emu(1059582), TXT_W, CONTENT_H)
    _set_box_bullets(txt, [
        'Если смотреть на один признак',
        'отдельно — это самый сильный.',
        '',
        '• Брони за неделю до заезда',
        '   отменяют примерно',
        '   в 10 % случаев',
        '',
        ...
        '',
        '• В модели вместе с другими',
        '   признаками порядок меняется',
        '   (см. слайд про важность).',
    ], size=12)
    ...
    _set_box_bullets(txt, [
        'Качество на тесте:',
        '   ROC-AUC = 0.917',
        '   F1-мера = 0.77, полнота 0.67',
        '',
        'Справа — значимость признаков',
        'по permutation importance.',
        '',
        'Страна вырывается вперёд:',
        'через целевое кодирование она',
        'несёт уникальную информацию.',
        '',
        'Срок до заезда коррелирует',
        'с каналом и сегментом — часть',
        'его сигнала модель',
        'восстанавливает через них.',
    ], size=11)
    ...
    # 13. ограничения и риски — кратко, акцент на риски
    s = work_slide('Ограничения модели', title_width=8500000)
    box = _textbox(s, LEFT_MARGIN, Emu(1059582), FULL_W, CONTENT_H)
    _set_box_bullets(box, [
        '1. Перенос на другие отели.',
        '   Обучались на двух отелях в Португалии.',
        '   На сетевой отель в Германии или Азии может не перенестись —',
        '   структура каналов и поведение клиентов другие.',
        '',
        '2. Смещение во времени.',
        '   Данные 2015 – 2017 годов. После ковида клиенты ведут себя иначе:',
        '   гибкие условия отмены стали нормой, доля отмен в индустрии выросла.',
        '',
        '3. Новая страна.',
        '   Если страны не было в обучении, целевое кодирование даёт среднее',
        '   по миру — точность для таких броней непредсказуемо хуже.',
        '',
        'Прототип развёрнут как Streamlit-приложение (предыдущий слайд).',
        'Для продакшена — веб-сервис, мониторинг распределений признаков',
        'и переобучение раз в квартал.',
    ], size=13)
